...
#[inline]
fn remote_host_peer_id(part_id: u64) -> u64 {
    part_id
}

#[inline]
fn remote_dpu_peer_id(part_id: u64) -> u64 {
    part_id + 100
}

pub struct OccHybrid<const MAX_ITEM_SIZE: usize>
{
    status:    OccStatus,
    part_id:   u64,
    tid:       u32,
...
// local operations
impl<const MAX_ITEM_SIZE: usize> OccHybrid<MAX_ITEM_SIZE>
{
                ...
                match item.rwtype {
                    RwType::ERASE => {
                        ...
                        self.batch_rpc.append_req(
                            &remote_req, 
                            remote_host_peer_id(item.part_id), 
                            0, 
    ...
    fn release_on(&mut self, update: bool) {
        ...
        for i in 0..ref_set.get_len() {
            ...
            if item.part_id == self.part_id {
                ...
                self.batch_rpc.append_req(
                    &remote_req, 
                    remote_dpu_peer_id(item.part_id), 
                    0, 
    ...
    fn process_fetch_write_resp(&mut self, resp_idx: usize, wrapper: &mut BatchRpcRespWrapper, num: u32) {
        for _ in 0..num {
            ...
            if item.update_idx >= self.updateset.get_len() {
                panic!("update length overflow???, resp_idx: {}, cid:{}, num:{}", resp_idx, self.cid, num);
            }
